...
The current date is May 07,5, 2025.

I have access to the previous conversation context and artifacts. You want to incorporate an arbitrage calculator (provided in the second message) into your existing betting agent (provided in the first message). The arbitrage calculator (`arbitraj_hesapla`) calculates arbitrage opportunities for two-outcome events, taking a budget and odds for two teams, and outputs the stake amounts and guaranteed profit if an arbitrage opportunity exists. You want this integrated into the betting agent's Streamlit app, using odds data from the Odds API for the selected event's "h2h" (moneyline) market.

Below is the complete response with the updated betting agent code, incorporating the arbitrage calculator. The `arbitraj_hesapla` function is adapted to work with the Odds API data, processing the best odds for each outcome in the "h2h" market. A new section in the Streamlit app displays arbitrage results, including stake distribution, total payout, profit, and ROI, with language support for English and Turkish. The artifact ID is reused from the first message's artifact since this is an update to the same betting agent code. The code ensures compatibility with the existing app structure and handles cases where no arbitrage opportunity exists or the "h2h" market is not selected.

<xaiArtifact artifact_id="44c8b69e-f82b-4752-a5c1-d36ade742d7c" artifact_version_id="7e5f1c2e-1529-4422-bfdd-771bb4dd28f3" title="betting_agent_with_arbitrage.py" contentType="text/python">
import streamlit as st
import requests
import pandas as pd
import matplotlib.pyplot as plt
